    ...
    <div className="space-y-10">
      ...
      <div className="glass-card shadow-premium border dark:border-white/5 rounded-[2.5rem] p-8">
        ...
        <div className="h-4 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden shadow-inner p-0.5">
          <div
            className="h-full bg-brand rounded-full transition-all duration-1000 ease-out shadow-[0_0_15px_rgba(var(--brand-clr),0.5)]"
        ...
        <div className="flex justify-between mt-3 px-1">
          ...
          <span className="text-[10px] font-black text-brand uppercase tracking-widest animate-pulse">{Math.round((monthVisits / repInfo.target) * 100)}{t('percentCompleted')}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 glass-card shadow-premium border dark:border-white/5 rounded-[2.5rem] p-8">
          <div className="flex items-center justify-between mb-8">
            ...
            <button onClick={() => navigate('/visits')} className="text-xs font-black uppercase tracking-widest text-brand hover:underline">{t('viewAll')}</button>
          </div>
          {recentVisits.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <Button onClick={() => navigate('/book')} className="bg-brand hover:bg-brand-dark text-brand-foreground h-12 rounded-2xl px-8 font-black uppercase tracking-widest shadow-lg shadow-brand/20">{t('bookNewVisit')}</Button>
            </div>
          ) : (
        ...
        <div className="space-y-6">
          <div className="glass-card shadow-premium border dark:border-white/5 rounded-[2.5rem] p-8">
            ...
            <div className="space-y-3">
              {[
                ...
                <Button key={label} onClick={() => navigate(href)} className={cn('w-full h-14 rounded-2xl justify-start gap-4 px-6 font-black uppercase tracking-widest italic shadow-lg transition-all active:scale-[0.98]', primary ? 'bg-brand hover:bg-brand-dark text-brand-foreground shadow-brand/20' : 'bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 dark:text-white text-slate-900 shadow-none')} variant="ghost">
          ...
          <div className="bg-brand/10 border border-brand/20 rounded-[2.5rem] p-8 relative overflow-hidden group shadow-premium transition-all hover:bg-brand/15">
            <div className="absolute -top-10 -right-10 w-32 h-32 bg-brand/10 rounded-full blur-3xl group-hover:bg-brand/20 transition-all"></div>
            <div className="flex items-center justify-between mb-4 relative z-10">
               <div className="text-[10px] font-black uppercase tracking-[0.2em] text-brand">{t('myCredits')}</div>
               <Button size="sm" variant="ghost" onClick={() => setShowTopup(true)} className="h-8 rounded-xl text-[9px] uppercase font-black tracking-[0.2em] text-brand hover:text-white hover:bg-brand bg-brand/10 px-3 border border-brand/20 transition-all">
                  {t('topUp')}
               </Button>
            </div>
              <div className="text-4xl font-black italic tracking-tighter text-brand relative z-10">
             ...
             <div className="text-[10px] font-bold text-brand/60 mt-2 uppercase tracking-widest relative z-10 italic">{t('availableForBookings')}</div>
          </div>
          {showTopup && (
               ...
               <div className="glass-card shadow-3xl border dark:border-white/10 rounded-[3rem] p-10 w-full max-w-md animate-in zoom-in-95 relative overflow-hidden">
                  ...
                  <div className="space-y-8 relative z-10">
                     <div className="space-y-3">
                        ...
                        <Input type="number" value={topupAmount} onChange={e => setTopupAmount(parseInt(e.target.value) || 0)} className="h-16 rounded-2xl bg-slate-50 dark:bg-white/5 border-slate-200 dark:border-white/10 text-2xl font-black italic px-8 focus:ring-4 focus:ring-brand/10 focus:border-brand transition-all" />
                     </div>
                     ...
                     <Button 
                       ...
                       className="w-full h-16 rounded-[2rem] bg-brand hover:bg-brand-dark text-brand-foreground font-black uppercase tracking-[0.2em] italic shadow-2xl shadow-brand/40 transition-all active:scale-[0.98]"
                     >
                        {t('confirmPayment')}
                     </Button>
                  </div>
