  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold dark:text-white">Manage Doctors</h1>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Add & manage doctors in your facility</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total Doctors', value: doctors.length, color: 'emerald' },
          { label: 'Cardiology', value: doctors.filter(d => d.specialty === 'Cardiology').length, color: 'red' },
          { label: 'Neurology', value: doctors.filter(d => d.specialty === 'Neurology').length, color: 'blue' },
          { label: 'Other', value: doctors.filter(d => !['Cardiology', 'Neurology'].includes(d.specialty)).length, color: 'amber' },
      ...
      {/* Search */}
      <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or specialty..." className="dark:bg-slate-800 dark:border-slate-600 dark:text-white dark:placeholder-slate-400 max-w-sm" />
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-md border dark:border-slate-700 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold dark:text-white">{editingDoc ? 'Edit Doctor' : 'Add Doctor'}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <Label className="dark:text-slate-300">Specialty</Label>
                <select
              ...
              <div>
                <Label className="dark:text-slate-300">Years of Experience</Label>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <Button type="submit" className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white">{editingDoc ? 'Update' : 'Add'} Doctor</Button>
              </div>
            </form>
          </div>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-slate-800/30 border dark:border-slate-700 rounded-xl p-12 flex flex-col items-center text-center">
          ...
          <h3 className="text-lg font-semibold dark:text-slate-300">{search ? 'No doctors match your search' : 'No Doctors Added Yet'}</h3>
          {!search && <Button onClick={() => setShowForm(true)} className="mt-4 bg-emerald-600 hover:bg-emerald-700 text-white gap-1"><Plus className="h-4 w-4" /> Add First Doctor</Button>}
        </div>
      ) : (
          ...
          {filtered.map(doc => (
            <div key={doc.id} className="bg-white dark:bg-slate-800/50 border dark:border-slate-700 rounded-xl p-5 hover:shadow-lg transition-shadow">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <Badge variant="outline" className="text-xs mt-0.5 border-blue-200 dark:border-blue-500/30 text-blue-600 dark:text-blue-400">{doc.specialty}</Badge>
                  </div>
                </div>
              ...
              <div className="space-y-1.5 text-xs text-gray-500 dark:text-slate-400">
                <div className="flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" />{doc.experienceYears} years experience</div>
              ...
              <div className="mt-3 pt-3 border-t dark:border-slate-700 flex items-center gap-2">
                ...
                <span className="ml-auto text-xs text-gray-400 dark:text-slate-500">{doc.availability.filter(s => !s.isBooked).length} slots available</span>
              </div>
            </div>
          ))}
        </div>
