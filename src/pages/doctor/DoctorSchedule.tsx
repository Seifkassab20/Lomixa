    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Manage your availability for pharma visits</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Available', value: slots.filter(s => !s.isBooked).length },
          { label: 'Booked', value: slots.filter(s => s.isBooked).length },
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-md border dark:border-slate-700 shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold dark:text-white">Add Availability Slot</h2>
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div>
                <Label className="dark:text-slate-300 mb-2 block">Appointment Type</Label>
              ...
              <div>
                <Label className="dark:text-slate-300">Duration (minutes)</Label>
                <select
                  value={form.duration}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <Button type="submit" className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white">Add Slot</Button>
              </div>
            </form>
          </div>
        </div>
      ...
      {/* Upcoming Slots */}
      <div>
        <h2 className="text-base font-semibold dark:text-white mb-3">Upcoming Slots</h2>
        {upcoming.length === 0 ? (
          <div className="bg-white dark:bg-slate-800/30 border dark:border-slate-700 rounded-xl p-10 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-slate-400">No upcoming slots. Add your availability to start receiving visits.</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-medium dark:text-white">{slot.appointmentType}</span>
                      ...
                      <div className="flex items-center gap-3 mt-0.5 text-xs text-gray-500 dark:text-slate-400">
                        <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{new Date(slot.date).toLocaleDateString('en-SA', { weekday: 'short', month: 'short', day: 'numeric' })}</span>
